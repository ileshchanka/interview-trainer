...
- **Просмотр колоды подряд** — тот же корпус без оценок и расписания: номер вопроса,
  переход к любому из списка, «Далее» и стрелки. Номер вопроса виден в адресе
  (`/browse/js/12`), поэтому на вопрос можно дать ссылку; вход без номера
  открывает то место, где остановились в прошлый раз.
