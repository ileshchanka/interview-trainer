...
Три режима в одном приложении:

- **Карточки с интервальным повторением** (алгоритм SM-2) — 537 вопросов с ответами,
  примерами кода и разборами. Чем увереннее ответ, тем дольше карточка не вернётся.
  Пример кода есть у каждой карточки обоих треков.
- **Просмотр колоды подряд** — тот же корпус без оценок и расписания: номер вопроса,
  переход к любому из списка, «Далее» и стрелки. Вопрос виден в адресе
  (`/browse/js/js-event-loop-order`), поэтому на него можно дать ссылку;
  вход без него открывает то место, где остановились в прошлый раз.
